...
const INITIAL_BOOKINGS: Booking[] = [
  { id: "1", serviceName: "Website Development", date: "2026-09-15", time: "10:00 AM", status: "pending", notes: "Need a landing page for a SaaS product" },
  { id: "2", serviceName: "Discord Bot Development", date: "2026-09-10", time: "02:00 PM", status: "confirmed", notes: "Moderation bot with ticket system" },
  { id: "3", serviceName: "UI/UX Design", date: "2026-08-28", time: "11:00 AM", status: "completed" },
];

const INITIAL_MESSAGES: Msg[] = [
  { id: "1", content: "Thanks for the quick turnaround on the website! The design looks great.", createdAt: "2026-08-30T14:30:00Z", serviceSlug: "website-development" },
  { id: "2", content: "Can we schedule a follow-up for the bot deployment?", createdAt: "2026-08-29T09:15:00Z", serviceSlug: "discord-bot-development" },
];
...
export default function Dashboard() {
  ...
  const [messages] = useState<Msg[]>(INITIAL_MESSAGES);
                  ...
                  {bookings.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {bookings.slice(0, 5).map((b) => {
                        ...
                        return (
                          <div key={b.id} className="flex items-center gap-4 rounded-2xl border border-border/40 bg-card/30 p-4 transition-all hover:bg-card/40">
                ...
                {bookings.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {bookings.map((b) => {
                      const StatusIcon = statusIcons[b.status] ?? Clock;
                      return (
                        <div key={b.id} className="flex items-center gap-4 rounded-2xl border border-border/40 bg-card/30 p-5 transition-all hover:bg-card/40">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {b.status === "pending" && (
                              <button onClick={() => cancelBooking(b.id)} className="text-xs text-red-400 hover:text-red-300">Cancel</button>
                            )}
                          </div>
                        </div>
                      );
            ...
            {activeTab === "messages" && (
              <div>
                ...
                {messages.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {messages.map((m) => (
                      <div key={m.id} className="rounded-2xl border border-border/40 bg-card/30 p-5 transition-all hover:bg-card/40">
