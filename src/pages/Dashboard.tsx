...
import {
  ...
  Clock, CheckCircle, XCircle, Loader2, ArrowRight, Package,
} from "lucide-react";
            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-border/40 bg-card/30 p-5">
                  ...
                  <p className="mt-1 text-2xl font-bold gradient-text">{messages?.length ?? 0}</p>
                </div>
              </div>
                ...
                {bookings && bookings.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {bookings?.slice(0, 5).map((b) => {
                      ...
                      return (
                        <div key={b._id} className="flex items-center gap-4 rounded-2xl border border-border/40 bg-card/30 p-4">
              ...
              {bookings && bookings.length === 0 ? (
                ...
                <div className="space-y-3">
                  {bookings?.map((b) => {
                    const StatusIcon = statusIcons[b.status] ?? Clock;
                    return (
                      <div key={b._id} className="flex items-center gap-4 rounded-2xl border border-border/40 bg-card/30 p-5">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          {b.status === "pending" && (
                            <button onClick={() => cancelBooking({ id: b._id })} className="text-xs text-red-400 hover:text-red-300">Cancel</button>
                          )}
                        </div>
                      </div>
                    );
          ...
          {activeTab === "messages" && (
            <div>
              ...
              {messages && messages.length === 0 ? (
                ...
                <div className="space-y-3">
                  {messages?.map((m) => (
                    <div key={m._id} className="rounded-2xl border border-border/40 bg-card/30 p-5">
                      <p className="text-sm leading-relaxed">{m.content}</p>
                      <p className="mt-2 text-[10px] text-muted-foreground">{new Date(m.createdAt).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              )}
